 ...
 *      如果 m > i，即元素值大于它的下标，则对于任意大于 0 的数 k，位于下标 i + k 的数字的值大于或等于元素 m + k；
 *          也就是 arr[i + k] 大于或等于 m + k，又由于 m > i，则 m + k > i + k；
 *          也就是说，如果第 i 的数字的值大于 i，则它右边的数字都大于对应的下标；
 *          此时，只需要从左部分的数组中找就可以了。
 *      如果 m < i，则情况正好相反，只需要从右部分的数组中找就可以了。
 *
 */
public class Solution {
    ...
    public static void main(String[] args) {
        int[] arr = {-3, -1, 1, 3, 5};
        System.out.println(integerIdenticalToIndex(arr));
    }
}
